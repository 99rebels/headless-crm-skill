#!/usr/bin/env python3
"""
render_import.py — render the import write-plan as a self-contained HTML approval preview, in the
shared "Ledger" identity. Purpose-built for a BULK review (not the per-item scrutiny the enrichment
digest is for): summary tiles up top, a "how I read your columns" mapping card (the thing you're
really approving), then tab-switched compact tables of the records — People / Companies / Deals.

Takes build_import.py's output (the whole object, or just its `digest` block). Approval happens in
the conversation, not here (an artifact can't call the CRM tools back).

Usage:
    python3 render_import.py plan.json out.html
    cat plan.json | python3 render_import.py - out.html
"""
...
def render_mapping(mapping: list, skipped: list, aliases: list) -> str:
    ...
    if aliases:
        ...
        norm = (
            "<div class='norm'><span class='nlabel'>I also translated the labels in your file to "
            "your CRM&rsquo;s own terms:</span>"
            f"<div class='nchips'>{chips}</div></div>"
        )

    return (
        "<section class='card'>"
        "<h2>How I read your columns <span class='hint'>correct me before you approve</span></h2>"
        "<div class='maphdr'><span>Column in your file</span><span class='to'>&rarr;</span>"
...
def render_tabs(counts: dict) -> str:
    ...
    return (
        "<div class='tabs' role='tablist' aria-label='Records to import'>"
...
def _more_row(shown: int, total: int, cols: int) -> str:
    ...
    if extra <= 0:
        return ""
    return f"<tr class='more'><td colspan='{cols}'>+{extra} more will be imported</td></tr>"
...
def render_people(digest: dict) -> str:
    ...
    if not people:
        body = "<tr><td colspan='4' class='empty'>No people in this file.</td></tr>"
        foot = ""
    return (
...
def render_companies(digest: dict) -> str:
    ...
    if not orgs:
        body = "<tr><td colspan='2' class='empty'>No companies in this file.</td></tr>"
        foot = ""
    return (
...
def render_deals(digest: dict) -> str:
    ...
    if not deals:
        body = "<tr><td colspan='3' class='empty'>No deals in this file.</td></tr>"
        foot = ""
    return (
...
def render(data: dict) -> str:
    ...
    file_html = f" from <span class='file'>{esc(src)}</span>" if src else ""
    subline = (
        f"Read <b>{rows} row{'s' if rows != 1 else ''}</b>{file_html} — nothing is saved until you approve."
        if rows is not None else f"Read your file{file_html} — nothing is saved until you approve."
    )

    if total == 0:
        body = (
            "<div class='note'><span class='ic'>!</span><div class='t'>Nothing to import — no rows "
            "mapped to a person, company, or deal. Check the column mapping and try again.</div></div>"
        )
        approve = ""
    else:
        body = (
        ...
        approve = (
            ...
            f"<div class='how'>Reply <span class='say'>import</span> to create all {all_created} records — or tell me "
            "what to change (e.g. <span class='say'>skip the deals</span>, or fix a column above). Existing people or "
            "companies with the same email or domain are reused, never duplicated.</div></div>"
        )

    return TEMPLATE.replace("{{SUBLINE}}", subline).replace("{{BODY}}", body).replace("{{APPROVE}}", approve)
...
<body>
  <div class="wrap">
    ...
    <header>
      <span class="kicker">Import preview</span>
      <h1>Ready to import into your CRM</h1>
...
def main() -> None:
    if len(sys.argv) < 3:
        sys.exit("usage: render_import.py <plan.json|-> <out.html>")
